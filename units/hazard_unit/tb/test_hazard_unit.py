...
def model_load_use_stall(
    ...
    rs1_hazard = opcode_uses_rs1(if_id_opcode) and (id_ex_rd == if_id_rs1)
    rs2_hazard = opcode_uses_rs2(if_id_opcode) and (id_ex_rd == if_id_rs2)
...
async def test_hazard_unit_randomized(dut):
    random.seed(64)
    opcodes = [OP, OP_IMM, OP_32, OP_IMM_32, LOAD, STORE, BRANCH, JALR]
